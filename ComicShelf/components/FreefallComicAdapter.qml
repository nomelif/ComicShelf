...
Item {
    ...
    function getTitle(){
        if(page_url === "http://freefall.purrsia.com/")
            return html.split("<TITLE>")[1].split("</TITLE>")[0] + " (latest episode)"
        return "Freefall n. "+page_url.split(/f[cvf]0*/)[page_url.split(/f[cvf]/).length-1].split(".htm")[0];
    }

    function continueReading(){
        get_by_url(fio.getPrevPage("Freefall"))
    }
