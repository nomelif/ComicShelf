...
Item {
    ...
    function show_image(){
        ...
        comic_strip.state = "upperleft";
        comic_strip.scale = 1;
        activity.visible = false;
        comic_status.visible = false;
        activity.value = 0;
        updateTitle(adapter.getTitle());
    }
    ...
    PinchArea {
        ...
        property int tgt_x: 0;
        property int tgt_y: 0;
        property double prev_scale: 1;
        onPinchStarted: {
            tgt_x = comic_strip.x;
            tgt_y = comic_strip.y;
        }
        ...
        onPinchUpdated: {
            ...
            comic_strip.scale = pinch.scale * prev_scale;

            comic_strip.x = (-pinch.startCenter.x+pinch.center.x)+tgt_x*pinch.scale;
            comic_strip.y = (-pinch.startCenter.y+pinch.center.y)+tgt_y*pinch.scale;
            }
        }

        MouseArea {
            onWheel: {
            if((comic_strip.scale < 7 && wheel.angleDelta.y > 0) || (comic_strip.scale > 0.5 && wheel.angleDelta.y < 0))
                comic_strip.scale *= 1+((wheel.angleDelta.y / Math.abs(wheel.angleDelta.y))*0.2)
            }
            id: dragArea
            hoverEnabled: true
            anchors.fill: parent
        ...
        Image{
            ...
            states: [
                ...
                State{
                    ...
                    AnchorChanges {
                        target: comic_strip;
                        anchors.left: comic_strip.parent.left;
                        anchors.top: comic_strip.parent.top;
                    }
                }
